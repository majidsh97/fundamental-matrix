...
# student function
#tells us  which points are inliers according to a minimum matching distance.
def inliers_epipolar_constraint(p1: t_points, p2: t_points, F:t_homography, distance_threshold: float) -> np.array:
    ...
    p1h = np.pad(p1, ((0,0),(0,1)), constant_values=1 )
    p2h = np.pad(p2, ((0,0),(0,1)), constant_values=1 )
    p1_proj = (t_homography @ p1h.T).T

    # Step 2 - normalize the projected points (2 lines)
    p1_proj = p1_proj / p1_proj[:,2]

    # Step 3 - compute the distance (1 line)
    D = np.abs( p1_proj @ p2h.T)

    # Step 4 - check if the distance is smaller than t and return (2 lines)
    inliers = np.nonzero(D < distance_threshold)[0]
    return inliers


# student function
# Given 8 correct feature matches, the fundamental matrix can be computed with the 8-point
# algorithm.
def compute_fundamental_matrix(points1: t_points, points2: t_points) -> np.array:
    ...
    # px1qx1 px1qy1 px1 py1qx1 py1qy1 py1 qx1 qy1 1

    # TODO 2.2 Construct the 8x9 matrix A.
    px, py = points1[:,0] , points1[:,1]
    qx, qy = points2[:,0] , points2[:,1]
    A = np.vstack([px*qx , px*qy , px , py*qx , py*qy , py, qx ,qy , np.ones_like(px)]).T
    ...
    Uf, Sf, Vf = np.linalg.svd(F, full_matrices=False)
    Sf[-1] = 0
    F = np.dot(Uf, np.dot(np.diag(Sf), Vf))
    F = F/ F[2,2] # why normalize with the last element
...
def compute_essential_matrix(fundamental_matrix: t_homography, camera_parameters: t_camera_parameters) -> t_homography:
    ...
    e = camera_parameters.T @ fundamental_matrix @ camera_parameters
    e /= e[2,2]
    return  e
