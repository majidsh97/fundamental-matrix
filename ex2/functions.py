...
def filter_and_align_descriptors(f1: Tuple[t_points, t_descriptors], f2: Tuple[t_points, t_descriptors],
    ...
    nn1 = sorted_diffs[:, 1, np.newaxis]
    nn2 = sorted_diffs[:, 2, np.newaxis]
    ratios = nn1 / nn2
    significant = (ratios > similarity_threshold).squeeze()
    ...
    dest_loc = dest_pts[valid_dest_indices]
...
def ransac(src_features: Tuple[t_points, t_descriptors], dst_features: Tuple[t_points, t_descriptors], steps,
    ...
    for n in range(steps):
        ...
        curr_inlier_num = _get_inlier_count(src_subset,dst_subset, H, distance_threshold=distance_threshold)
...
def translate_homographies(homographies: t_homographies, dx: float, dy: float):
    ...
    disp_vector = np.array([dx, dy])
    T = np.vstack([np.eye(2), disp_vector]).T

    tranlated_homographies = {}
    # step 2: apply translation matrix on every homography matrix (2 lines)
    for k, h in list(homographies.items()):
        tranlated_homographies[k] = h



    return tranlated_homographies
